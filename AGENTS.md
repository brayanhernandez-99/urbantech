...
- Results rendered by JS into `#credit-results`; per-entity card shows `Recargo (X.X%)` (real % on cash price, derived from `additional / cash`) + additional in money + TOTAL (primary) + CTA "Quiero este crédito" via `openWhatsApp`.
